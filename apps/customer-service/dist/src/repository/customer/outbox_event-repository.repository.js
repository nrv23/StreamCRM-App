...
export class OutboxEventRepository {
    ...
    */
    ...
    async markAsPublished(eventId) {
        ...
        if (!response)
            throw ErrorFactory.build(ApiErrorCode.INTERNAL_SERVER_ERROR, `outbox_events with id ${eventId} not found`);
    }
    ...
    async save(event) {
        const [outBoxEventResponse] = await this._db.query(`
        ...
        if (!outBoxEventResponse)
            throw ErrorFactory.build(ApiErrorCode.CONFLICT_ERROR, "Event was not inserted");
    ...
    async findPending(limit) {
        const sql = `
            ...
            LIMIT $1;
        `;
        const response = await this._db.query(sql, [limit]);
        return response;
    }
}
//# sourceMappingURL=outbox_event-repository.repository.js.map
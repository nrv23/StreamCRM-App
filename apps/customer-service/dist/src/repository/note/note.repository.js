...
export class NoteRepository {
    ...
    async save(note) {
        const [newNote] = await this._db.query('Insert into customer_notes(customer_id,user_id,note) values($1,$2,$3) returning *', [
            note.customer_id, note.user_id, note.note
        ]);
        if (!newNote)
            throw ErrorFactory.build(ApiErrorCode.CONFLICT_ERROR, "Note was not inserted");
        return newNote;
    }
    ...
    `;
        const notes = await this._db.query(query, params);
        return notes;
    }
    // count de registros
